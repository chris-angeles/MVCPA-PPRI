...
If FiscalYear < 2023 Then
	Response.Redirect("Scores.asp?FiscalYear=" & FiscalYear)
...
&nbsp;&nbsp;&nbsp;<a href="Scores3.asp?ShowExcel=1&FiscalYear=<%=FiscalYear %>&OrderBy=<%=OrderBy %>&GrantTypeID=<%=GrantTypeID %>" target="_blank">Excel</a><br />
...
<%
sql = "SELECT A.AppID, A.ProgramName, C.GranteeName, A.GrantTypeID, REPLACE(B.GrantType, ' Grant','') AS GrantType " & vbCrLF & _
...
sql = "SELECT *, 1 AS AverageToEnd " & vbCrLF & _
	"FROM Scoring.vwScoringAverages3 WITH (NOLOCK) " & vbCrLF & _
...
sql = sql &	"UNION " & vbCrLF & _
	...
	"FROM Scoring.vwScores3 WITH (NOLOCK) " & vbCrLf & _
	"WHERE Fiscal_Year=" & prepIntegerSQL(FiscalYear) & " " & vbCrLF
	If GrantTypeID = 0 Then
		' Skip
	ElseIf GrantTypeID = 4 Then
		sql = sql & vbTab & "AND GrantTypeID IN (1,3) " & vbCrLf
	ElseIf GrantTypeID > 0 Then
		sql = sql & vbTab & "AND GrantTypeID=" & prepIntegerSQL(GrantTypeID) & " " & vbCrLf
	End If
...
If rs.EOF = False Then
	...
	Response.Write(vbTab & "<th>Q12</th>" & vbCrLf)
	Response.Write(vbTab & "<th>Q13</th>" & vbCrLf)
	Response.Write(vbTab & "<th>Q1-Q13</th>" & vbCrLf)
	Response.Write(vbTab & "<th>EC</th>" & vbCrLf)
	...
	While rs.EOF = False
		...
		If IsNull(rs.Fields("Score_12").Value) = True Then
			Response.Write(vbTab & "<td></td>" & vbCrLf)
		ElseIf IsNull(rs.Fields("Color_12")) = False Then
			Response.Write(vbTab & "<td style=""background-color: " & rs.Fields("Color_12") & "; text-align: right; "">" & rs.Fields("Score_12").Value & "</td>" & vbCrLf)
		Else
			Response.Write(vbTab & "<td style=""text-align: right; "">" & rs.Fields("Score_12").Value & "</td>" & vbCrLf)
		End If
		If IsNull(rs.Fields("Score_13").Value) = True Then
			Response.Write(vbTab & "<td></td>" & vbCrLf)
		ElseIf IsNull(rs.Fields("Color_13")) = False Then
			Response.Write(vbTab & "<td style=""background-color: " & rs.Fields("Color_13") & "; text-align: right; "">" & rs.Fields("Score_13").Value & "</td>" & vbCrLf)
		Else
			Response.Write(vbTab & "<td style=""text-align: right; "">" & rs.Fields("Score_13").Value & "</td>" & vbCrLf)
		End If

		Response.Write(vbTab & "<td style=""text-align: right; "">" & rs.Fields("Question 1-13 Total").Value & "</td>" & vbCrLf)
		Response.Write(vbTab & "<td style=""text-align: right; "">" & rs.Fields("Score_14").Value & "</td>" & vbCrLf)
