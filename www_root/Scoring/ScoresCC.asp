...
&nbsp;&nbsp;&nbsp;<a href="ScoresCC.asp?ShowExcel=1&FiscalYear=<%=FiscalYear %>&OrderBy=<%=OrderBy %>&GrantTypeID=<%=GrantTypeID %>" target="_blank">Excel</a><br />
...
sql = "SELECT A.AppID, A.ProgramName, C.GranteeName, A.GrantTypeID, REPLACE(B.GrantType, ' Grant','') AS GrantType " & vbCrLF & _
	...
	"WHERE I.GrantClassID=4 AND  I.FiscalYear=" & prepIntegerSQL(FiscalYear) & " AND SubmitTimestamp IS NOT NULL" & vbCrLf
...
sql = "SELECT *, 1 AS AverageToEnd " & vbCrLF & _
	"FROM Scoring.vwScoringAveragesCC WITH (NOLOCK) " & vbCrLF & _
...
sql = sql &	"UNION " & vbCrLF & _
	...
	"FROM Scoring.vwScoresCC WITH (NOLOCK) " & vbCrLf & _
	"WHERE Fiscal_Year=" & prepIntegerSQL(FiscalYear) & " " & vbCrLF
...
If rs.EOF = False Then
	...
	'Response.Write(vbTab & "<th>Q1-Q8</th>" & vbCrLf)
	...
	While rs.EOF = False
		...
		If IsNull(rs.Fields("Grant_Type").Value) = False Then
		...
		'Response.Write(vbTab & "<td style=""text-align: right; "">" & rs.Fields("Question 1-8 Total").Value & "</td>" & vbCrLf)
