...
If AppID=0 Or Button="submit" Or foundchange = true Then
	If AppID = 0 Then
		sql = "INSERT INTO Negotiation.Main (GranteeID, FiscalYear, ProgramName, GrantTypeID, " & vbCrLf & _
		...
		sql = sql & "	UpdateID, UpdateTimestamp) " & vbCrLf & _
			"VALUES (" & prepIntegerSQL(GranteeID) & ", " & _
		...
		End If
		Con.Execute(sql)
		sql = "SELECT IDENT_CURRENT('Negotiation.Main') AS AppID"
		If Debug = True Then
			Response.Write("<pre>" & sql & "</pre>" & vbCrLf)
			Response.Flush
		End If
		...
		End If
	Else
		sql = "UPDATE Negotiation.Main " & vbCrLf & "SET " & _
...
If ParticipatingAgenciesChanged = True Then
	...
	If Len(ParticipatingAgencies)>0 Then
		...
		sql = "DELETE FROM Negotiation.ParticipatingAgencies WHERE AppID=" & prepIntegerSQL(AppID) 
		If Len(ParticipatingAgencies)>0 Then
		...
		For i = 1 To Request.Form("ParticipatingAgencies").Count
			sql = "SELECT AppID, ORI FROM Negotiation.ParticipatingAgencies WHERE AppID=" & _
			...
			Set rs = con.execute(sql)
			IF rs.EOF = True Then
				sql = "INSERT INTO Negotiation.ParticipatingAgencies(AppID, ORI, UpdateID, UpdateTimeStamp) VALUES (" & _
...
If CoverageAgenciesChanged = True Then
	...
	If Len(CoverageAgencies)>0 Then
		...
		sql = "DELETE FROM Negotiation.CoverageAgencies WHERE AppID=" & prepIntegerSQL(AppID) 
		If Len(CoverageAgencies)>0 Then
			sql = sql & " AND ORI NOT IN (" & List & ")"
		End If
		If Debug = True Then
		...
		For i = 1 To Request.Form("CoverageAgencies").Count
			sql = "SELECT AppID, ORI FROM Negotiation.CoverageAgencies WHERE AppID=" & _
			...
			Set rs = con.execute(sql)
			IF rs.EOF = True Then
				sql = "INSERT INTO Negotiation.CoverageAgencies(AppID, ORI, UpdateID, UpdateTimeStamp) VALUES (" & _
	...
	While rs.EOF = False
		...
		If InStr(1,Changes, "Question_" & SectionTextID)>0 Then
			...
			ElseIf Len(SectionText)>0 And rs.Fields("RecordPresent") = False Then
				sql = "INSERT INTO Negotiation.SectionText (TextSectionID, AppID, SectionText, UpdateID, UpdateTimestamp)" & vbCrLf & _
			...
			ElseIf Len(SectionText) > 0 and rs.Fields("RecordPresent") = True Then
				sql = "UPDATE Negotiation.SectionText SET " & vbCrLf & _
			...
			ElseIf Len(SectionText)=0 And IsNull(rs.Fields("SectionText")) = False Then 
				sql = "DELETE FROM Negotiation.SectionText " & vbCrLf & _
					"WHERE TextSectionID=" & SectionTextID &" AND AppID=" & prepIntegerSQL(AppID)
				If Debug = True Then
					Response.Write("<pre>" & sql & "</pre>" & vbCrLf)
					Response.Flush
...
If FiscalYEar>2019 And BudgetEntryOption="2" Then
	sql = "DECLARE @BudgetCashMatch FLOAT = " & prepNumberSQL(BudgetCashMatch) & "/100.0;" & vbCrLf & _
		"UPDATE Negotiation.BudgetDetails " & vbCrLf & _
