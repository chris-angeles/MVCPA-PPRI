...
Dim debug, i, j, k, PermitEdit, AppID, GrantClassID, GrantClass, MatchTypeID, MatchType, GranteeID, GranteeName, FiscalYear, _
	SubmitID, SubmitName, SubmitTimestamp, NoOfOptions, RoundCurrency, AppURL, NegostiationLocked
...
If Debug = True Then
	Response.Write("<pre>Made it to Line 33</pre>" & vbCrLf)
	Response.Flush
End If
...
sql = "SELECT I.AppID, I.GrantClassID, D.GrantClass, I.FiscalYear, G.GranteeID, G.GranteeName, " & vbCrLf & _
	...
	"	CAST(ISNULL(NegotiationLocked,0) AS Bit) AS NegostiationLocked, " & vbCrLf & _
	...
	"LEFT JOIN Negotiation.Main AS A ON A.AppID=I.AppID " & vbCrLF & _
...
If rs.EOF = False Then
	...
	NegostiationLocked = rs.Fields("NegostiationLocked")
Else
...
If GranteeID>0 Then
	If NegostiationLocked = True Then
		PermitEdit = False 
...
<html lang="en-us">
<head>
<title>MVCPA Grant Application Negotiation for <%=GranteeName %>: <%=MatchType %></title>
...
<%=HiddenField("AppID", AppID) %><%=HiddenField("MatchTypeID",MatchTypeID) %><%=HiddenField("FiscalYear",FiscalYear) %><%=HiddenField("ButtonChoice", "save") %>
