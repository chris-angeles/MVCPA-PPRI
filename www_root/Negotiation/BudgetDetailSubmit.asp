...
For i = 1 to RowCount
	...
	If Len(MVCPAFunds)>0 Or Len(CashMatch)>0 Or Len(Description)>0 Or Len(InKindMatch)>0 Then
		If CInt(BudgetItemID) = 0 Then ' Insert
			sql  = "INSERT INTO Negotiation.BudgetDetails (AppID, BudgetCategoryID, Description, NoOfItems, " & vbCrLf & _
			...
			Con.Execute(sql)
		Else ' Update
			sql = "UPDATE Negotiation.BudgetDetails SET " & vbCrLF & _
	...
	ElseIf Cint(BudgetItemID)>0 Then ' Delete
		sql = "DELETE FROM Negotiation.BudgetDetails WHERE BudgetItemID=" & prepIntegerSQL(BudgetItemID)
		If Debug = True Then
			Response.Write("<pre>" & sql & "</pre>" & vbCrLf)
			Response.Flush
		End If
...
If Len(Narrative)>0 Then
	sql = "UPDATE Negotiation.BudgetDetailNarrative SET " & vbCrLF & _
		...
		"INSERT INTO Negotiation.BudgetDetailNarrative (BudgetCategoryID, AppID, Narrative, UpdateID, UpdateTimestamp) " & vbCrLF& _
...
Else
	sql = "DELETE FROM Negotiation.BudgetDetailNarrative WHERE AppID=" & prepIntegerSQL(AppID) & " AND BudgetCategoryID=" & BudgetCategoryID
		If Debug = True Then
			Response.Write("<pre>" & sql & "</pre>" & vbCrLf)
			Response.Flush
		End If
