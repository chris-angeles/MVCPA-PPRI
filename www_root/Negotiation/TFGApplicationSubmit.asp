...
If AppID=0 Or Button="submit" Or foundchange = true Then
	If AppID = 0 Then
		sql = "INSERT INTO " & ApplicationSchema & ".Main (GranteeID, FiscalYear, ProgramName, CoverageAreaDescription, GrantTypeID, " & vbCrLf & _
		...
		End If
		Con.Execute(sql)
		sql = "SELECT IDENT_CURRENT('" & ApplicationSchema & ".Main') AS AppID"
		If Debug = True Then
			Response.Write("<pre>" & sql & "</pre>" & vbCrLf)
			Response.Flush
		End If
		...
		If rs.EOF = True Then
			...
			sendWarning("Error: No Grantee and " & ApplicationSchema & " record retrieved")
			Response.End
		Else
