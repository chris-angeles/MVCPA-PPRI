...
If AppID>0 Then 
	sql = "SELECT G.GranteeID, G.GranteeName, G.ORI, G.OrganizationTypeID, G.StatePayeeIDNo, " & vbCrLf & _
		...
		"LEFT JOIN Application.IDs AS I ON I.GranteeID=I.GranteeID " & vbCrLf & _
...
Else
	sql = "SELECT G.GranteeID, G.GranteeName, G.ORI, G.OrganizationTypeID, G.StatePayeeIDNo, " & vbCrLf & _
		"	ISNULL(I.FiscalYear, " & prepIntegerSQL(FiscalYear) & ") AS FiscalYear, " & vbCrLf & _
		"	AuthorizedOfficialID, ISNULL(I.AppID,0) AS AppID, ProgramName, " & vbCrLf & _
		...
		"FROM Grantees AS G " & vbCrLf & _
		"LEFT JOIN Application.IDs AS I ON I.GranteeID=G.GranteeID AND I.FiscalYear=" & prepIntegerSQL(FiscalYear) & " SND GrantClassID=1 " & vbCrLf
		"LEFT JOIN Application.Main AS A ON A.AppID=I.AppID " & vbCrLf
...
<script type="text/javascript">
	function submitForm(action)
	{
		...
		<% If IsNull(TargetAwardAmount) = False Then %>
		if (document.Application.GrantTypeID.checked==false) {
...
<div class="pagetag"><%=GranteeName %> Grant Application for Fiscal Year <%=FiscalYear %></div>
...
<%
	sql = "SELECT A.ORI, REPLACE(A.Agency,'&','&amp;') AS Agency, B.County, A.CountyID " & vbCrLf & _
		...
		"WHERE A.ORI NOT IN (SELECT ORI FROM Application.ParticipatingAgencies WHERE AppID=" & prepIntegerSQL(AppID) & ") " & vbCrLf & _
		"	AND A.ORI NOT IN (SELECT ORI FROM Application.CoverageAgencies WHERE AppID=" & prepIntegerSQL(AppID) & ") " & vbCrLf & _
...
sql = "SELECT A.TextSectionID, A.Section, A.SubSection, A.SectionTitle, A.QuestionPreText, " & vbCrLf & _
	...
	"LEFT JOIN Application.SectionText AS B ON A.TextSectionID=B.TextSectionID AND B.AppID=" & prepIntegerSQL(AppID) & " " & vbCrLf & _
