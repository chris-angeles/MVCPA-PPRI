...
Else
	AppURL = getHomeApplicationReferenceByAppID(AppID)
End If
...
For i = 1 to RowCount
	...
	If Len(Source)>0 Or Len(Amount)>0 Then
		If CInt(MatchID) = 0 Then ' Insert
			sql  = "INSERT INTO Application.Matches (AppID, MatchTypeID, Source, MatchSourceID, Amount, " & vbCrLf & _
			...
			Con.Execute(sql)
		Else ' Update
			sql = "UPDATE Application.Matches SET " & vbCrLF & _
	...
	ElseIf Cint(MatchID)>0 Then ' Delete
		sql = "DELETE FROM Application.Matches WHERE MatchID=" & prepIntegerSQL(MatchID)
		If Debug = True Then
			Response.Write("<pre>" & sql & "</pre>" & vbCrLf)
			Response.Flush
		End If
