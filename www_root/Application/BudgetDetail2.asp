...
Dim debug, i, j, k, PermitEdit, AppID, GrantClassID, GrantClass, GranteeID, GranteeName, FiscalYear, _
	BudgetCategoryID, BudgetCategory, CategoryDescription, Narrative, NoOfOptions, _
	BudgetCashMatch, RoundCurrency, _
	SubmitID, SubmitByName, SubmitTimestamp, AppURL
...
sql = "SELECT I.AppID, I.GrantClassID, D.GrantClass, I.FiscalYear, G.GranteeID, G.GranteeName, " & vbCrLf & _
	...
	"LEFT JOIN Application.BudgetDetailNarrative AS C ON C.BudgetCategoryID=" & prepIntegerSQL(BudgetCategoryID) & _
...
<html lang="en-us">
<head>
<title>MVCPA Grant Application for <%=GranteeName %></title>
...
<%	End If %>
	...
	function updateTotals()
	{
		...
		var BudgetCashMatch = <%=BudgetCashMatch/100%>;
		var MVCPAFunds, CashMatch;
		for (i = 1; i <= document.BudgetDetail.RowCount.value; i++)
		{
			var rowtotal = 0;
			rowtotal = getNumericValue(document.BudgetDetail["LineTotal_"+i].value);
			MVCPAFunds = Math.round(rowtotal / (1 + BudgetCashMatch));
			CashMatch = Math.round(rowtotal * BudgetCashMatch / (1 + BudgetCashMatch));
			if (MVCPAFunds==0)
				document.BudgetDetail["MVCPAFunds_"+i].value = "";
			else 
			{
		//alert(MVCPAFunds + " = " + currencyRound(MVCPAFunds, true) + " total=" + (MVCPAFunds+MVCPAFunds_Total));
		document.BudgetDetail["MVCPAFunds_"+i].value = currencyRound(MVCPAFunds, <%=LCase(CStr(RoundCurrency))%>);
			}
			if (CashMatch==0)
				document.BudgetDetail["CashMatch_"+i].value = "";
			else
				document.BudgetDetail["CashMatch_"+i].value = currencyRound(CashMatch, <%=LCase(CStr(RoundCurrency))%>);
			MVCPAFunds_Total = MVCPAFunds_Total + MVCPAFunds;
			CashMatch_Total = CashMatch_Total + CashMatch;
...
Response.Write(HiddenField("BudgetCashMatch", BudgetCashMatch))
...
Response.Write("<p style=""margin: auto; width: 75%; text-align: left"">Specified Match Percentage: " & BudgetCashMatch & "%</p>" & vbCrLf)
Response.Write("<br />" & vbCrLf)

sql = "SELECT A.BudgetItemID, A.AppID, A.BudgetCategoryID, A.Description, A.NoOfItems, A.SubCategoryID, B.SubCategory, A.PctTime, A.PctSalary, A.MVCPAFunds, A.CashMatch, A.InKindMatch, A.LineTotal " & vbCrLF & _
	"FROM Application.BudgetDetails AS A" & vbCrLF & _
...
while rs.EOF = False
	i = i + 1
	WriteBudgetRow rs.Fields("BudgetItemID"), rs.Fields("BudgetCategoryID"), i, rs.Fields("Description"), rs.Fields("NoOfItems"), rs.Fields("SubCategoryID"), rs.Fields("PctTime"), rs.Fields("PctSalary"), rs.Fields("MVCPAFunds"), rs.Fields("CashMatch"), rs.Fields("InKindMatch"), rs.Fields("LineTotal"), PermitEdit
	rs.MoveNext
Wend

For j = 1 to 5
	i = i + 1
	WriteBudgetRow 0, BudgetCategoryID, i, "", "", "", "100", "", "", "", "", 0, PermitEdit
Next
...
<%	If PermitEdit = True Then %>
	...
	<input type="button" value="Cancel" onclick="location.href='<%=getHomeApplicationReferenceByGrantClass(GrantClassID, AppID) %>';" 
		title="Return to the application without saving latest changes." />
<%	Else %>
	<input type="button" value="Return" onclick="location.href='<%=getHomeApplicationReferenceByGrantClass(GrantClassID, AppID) %>';" 
...
Sub WriteBudgetRow(vBudgetItemID, vBudgetCategoryID, i, vTitle, vNoOfItems, vSubCategoryID, vPctTime, vPctSalary, vMVCPAFunds, vCashMatch, vInKindMatch, vLineTotal, vPermitEdit)
	Response.Write("<tr>" & vbCrLf)
	Response.Write(WriteInCell(TextField("Description_" & i, vTitle, 38, 255, vPermitEdit, "")))
	If vBudgetCategoryID=7 Then
		Response.Write(WriteInCell(IntegerField("NoOfItems_" & i, vNoOfItems, 2, 3, vPermitEdit, "return checkInteger(this);")))
	End If
	...
	End If	
	If BudgetCategoryID=1 Or BudgetCategoryID=4 Then
		Response.Write(WriteInCell(IntegerField("PctTime_" & i, vPctTIme, 2, 3, vPermitEdit, "return checkInteger(this);")&"%"))
	End If
	If vBudgetCategoryID=0 Then
		Response.Write(WriteInCell(IntegerField("PctSalary_" & i, vPctSalary, 2, 3, vPermitEdit, "return checkInteger(this);")&"%"))
	End If
	Response.Write(WriteInCell(CurrencyFieldRound("MVCPAFunds_" & i, vMVCPAFunds, 10, 11, RoundCurrency, False, "")))
	Response.Write(WriteInCell(CurrencyFieldRound("CashMatch_" & i, vCashMatch, 10, 11, RoundCurrency, False, "")))
	Response.Write(WriteInCell(CurrencyFieldRound("LineTotal_" & i, vLineTotal, 10, 11, RoundCurrency, vPermitEdit, "changedCurrencyField(this);") & vbCrLf _
		& HiddenField("BudgetItemID_" & i, vBudgetItemID)))
	Response.Write(WriteInCell(CurrencyFieldRound("InKindMatch_" & i, vInKindMatch, 10, 11, RoundCurrency, vPermitEdit, "changedCurrencyField(this);")))
	If PermitEdit = True Then
