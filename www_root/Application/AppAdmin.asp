...
If ShowOnlySubmitted = True Then
	sql = sql & "WHERE I.GrantClassID=1 AND (I.FiscalYear=" & prepIntegerSQL(FiscalYear) & " AND SubmitTimestamp IS NOT NULL) OR A.AppID=" & prepIntegerSQL(AppID) & " " & vbCrLf
Else
	sql = sql & "WHERE I.GrantClassID=1 AND I.FiscalYear=" & prepIntegerSQL(FiscalYear) & " OR I.AppID=" & prepIntegerSQL(AppID) & " " & vbCrLf
End If
...
sql = "SELECT I.AppID, G.GranteeName, G.ORI, O.Agency, A.ProgramName, I.GrantClassID, GC.GrantClass, I.FiscalYear, A.GrantTypeID, T.GrantType, " & vbCrLf & _
	...
	"FROM Application.IDs AS I " & vbCrLF & _
	"LEFT JOIN Application.Main AS A ON A.AppID=I.AppID " & vbCrLf & _
	"LEFT JOIN Application.Admin AS B ON B.AppID=I.AppID " & vbCrLf & _
	...
	"LEFT JOIN Negotiation.Main AS A2 ON A2.AppID=I.AppID " & vbCrLf & _
...
<tr>
	...
	While rs.EOF = False
		If ProgramCategory(rs.Fields("ProgramCategoryID")) = True Then
			Response.Write(vbTab & "&bullet; " & rs.Fields("ProgramCategory") & "<br />" & vbCrLf)
		End If
		rs.MoveNext
	Wend
%></td>
...
<tr style="vertical-align: top; ">
	...
	by <%=InterlocalAgreementConfirmedByName %>
	</td>
</tr>
<tr style="vertical-align: top; ">
	<td>Date that Prosecutor Agreement(s) are confirmed:</td>
	<td><%=DateField("ProsecutorAgreementConfirmedDate", ProsecutorAgreementConfirmedDate, True) %><%=HiddenField("ProsecutorAgreementConfirmedID", ProsecutorAgreementConfirmedID) %> 
	by <%=ProsecutorAgreementConfirmedByName %>
	</td>
</tr>
...
<tr>
	<td><a href="../Application/TFGPrintApplication.asp?AppID=<%=AppID%>" target="DisplayApp" class="plainlink">Original Application</a></td>
	<td style="width: 400px; "></td>
	<td><a href="../Negotiation/TFGPrintApplication.asp?AppID=<%=AppID%>" target="DisplayApp" class="plainlink">Revised Application</a></td>
</tr>
<%	Else %>
<tr>
	<td><a href="../Application/PrintApplication.asp?AppID=<%=AppID%>" target="DisplayApp" class="plainlink">Original Application</a></td>
	<td style="width: 400px; "></td>
	<td><a href="../Negotiation/PrintApplication.asp?AppID=<%=AppID%>" target="DisplayApp" class="plainlink">Revised Application</a></td>
</tr>
<%	End If %>
</table>
<%	End If %>
...
If FiscalYear > 2021 Then
	Response.Write("../Application/TFGPrintApplication.asp?AppID=" & AppID)
Else
