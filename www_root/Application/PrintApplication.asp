...
If FiscalYear > 2021 Then
	Response.Redirect("TFGPrintApplication.asp?AppID=" & AppID & "&FiscalYear=" & FiscalYear & "&GranteeID=" & GranteeID)
End If
If AppID>0 Then 
	sql = "SELECT G.GranteeID, G.GranteeName, ISNULL(I.FiscalYear, " & prepIntegerSQL(FiscalYear) & _
		...
		"LEFT JOIN Application.TargetAmounts AS N ON N.AppID=I.AppID " & vbCrLf & _
...
Else
	sql = "SELECT G.GranteeID, G.GranteeName, ISNULL(A.FiscalYear, " & prepIntegerSQL(FiscalYear) & _
		...
		"LEFT JOIN System.Users AS U ON U.SystemID=A.SubmitID " & vbCrLf & _
		"LEFT JOIN System.Users AS AO ON AO.SystemID=G.AuthorizedOfficialID " & vbCrLF & _
		"LEFT JOIN Application.TargetAmounts AS N ON N.AppID=A.AppID " & vbCrLf & _
...
<head>
<title><%=GranteeName %> Grant Application for Fiscal Year <%=FiscalYear %></title>
...
sql = "SELECT ISNULL(A.BudgetCategoryID,99) AS BudgetCategoryID, ISNULL(A.BudgetCategory, 'Total') As BudgetCategory, " & vbCrLF & _
	...
	If IsNull(TargetAwardAmount) = False And IsNull(TotalMVCPAFunds) = False Then
		If TotalMVCPAFunds > TargetAwardAmount Then
			Response.Write(vbTab & "<tr><td colspan=""5"" style=""text-align: center; color: red; "">The maximum award for this grant is " & formatcurrency(TargetAwardAmount) & ".</td</tr>" & vbCrLf)
...
sql = "SELECT B.BudgetItemID, A.BudgetCategoryID, A.BudgetCategory, " & vbCrLf & _
	...
	"FROM Lookup.BudgetCategories AS A " & vbCrLf & _
	"LEFT JOIN Application.BudgetDetails AS B ON B.BudgetCategoryID=A.BudgetCategoryID AND AppID=" & prepIntegerSQL(AppID) & " " & vbCrLf & _
...
sql = "SELECT A.BudgetCategoryID, A.BudgetCategory, B.Narrative " & vbCrLf & _
	"FROM Lookup.BudgetCategories AS A " & vbCrLf & _
	"JOIN Application.BudgetDetailNarrative AS B ON B.BudgetCategoryID=A.BudgetCategoryID AND B.AppID=" & prepIntegerSQL(AppID)
If Debug = True Then
	Response.Write("<pre>" & sql & "</pre>" & vbCrLf)
	Response.Flush
End If
...
sql = "SELECT A.TextSectionID, A.Section, A.SubSection, A.SectionTitle, A.QuestionPreText, " & vbCrLf & _
	...
	"LEFT JOIN Application.SectionText AS B ON A.TextSectionID=B.TextSectionID AND B.AppID=" & prepIntegerSQL(AppID) & " " & vbCrLf
	If FiscalYear < 2022 Then
		sql = sql & "WHERE A.TextSectionID<>13 and A.Version=1 " & vbCrLf
	Else
		sql = sql & "WHERE A.Version=1 " & vbCrLf
	End If
	sql = sql & "ORDER BY Section, SubSection "
If Debug = True Then
	Response.Write("<pre>" & sql & "</pre>" & vbCrLf)
	Response.Flush
End If
...
Function outputGSA()
	...
	vsql = "SELECT G.GoalID, S.StrategyID, A.ActivityID, A.MeasureID AS MeasureID, " & vbCrLf & _
		...
		"LEFT JOIN Application.GSATargets AS T ON T.AppID=" & prepIntegerSQL(AppID) & " AND T.GoalID=G.GoalID AND T.StrategyID=S.StrategyID AND T.ActivityID=A.ActivityID AND T.MeasureID=A.MeasureID " & vbCrLF & _
...
vsql = "SELECT G.GoalID, S.StrategyID, A.ActivityID, A.MeasureID AS MeasureID, " & vbCrLf & _
	...
	"LEFT JOIN Application.GSATargets AS T ON T.AppID=" & prepIntegerSQL(AppID) & " AND T.Version=G.Version AND T.GoalID=G.GoalID AND T.StrategyID=S.StrategyID AND T.ActivityID=A.ActivityID AND T.MeasureID=A.MeasureID " & vbCrLF & _
