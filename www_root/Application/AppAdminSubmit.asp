...
Dim debug, i, j, AppID, NewAppID, FiscalYear, Changes, _
...
If Len(Changes) > 2 Or ClearSubmit = True Or RevisedClearSubmit = True Then
	...
	sql = "SELECT * FROM Application.Admin WHERE AppID=" & prepIntegerSQL(AppID)
	If Debug = True Then
	...
	Else
		' Update
		sql = "UPDATE [Application].[Admin] " & vbCrLf & _
			...
			"	[NegotiationLocked] = " & prepBitSQL(NegotiationLocked) & ", " & _
...
If CreateGrantRecord = True Or CreateStubRecord = True Then
	sql = "INSERT INTO [Grants].Main (GrantClassID, FiscalYear, GranteeID, GrantNumber, POIssueDate, ProgramName, " & vbCrLf & _
		...
		"SELECT I.GrantClassID, I.FiscalYear, I.GranteeID, B.GrantNumber, A.POIssueDate, A.ProgramName, " & vbCrLf & _
		...
		"FROM " & ApplicationSchema & ".Main AS A " & vbCrLf & _
	...
	sql = "MERGE [Grants].Main AS [Target] " & vbCrLf & _
		...
		"		D.InLieuOfDPSBudget, D.InLieuOfNICBBudget, D.ProgramIncomeBudget, " & vbCrLf & _
		"		ReimbursementRate = 100.0 * B.GrantAwardAmount / (ISNULL(B.GrantAwardAmount,0.0) + ISNULL(M.CashMatch,0.0) - ISNULL(D.InLieuOfDPSBudget,0.0) - ISNULL(D.InLieuOfNICBBudget,0.0)), " & vbCrLf & _
		"		A.AppID, PY.GrantID AS PreviousYEarGrantID, " & vbCrLf & _
		...
		"		GROUP BY AppID) AS D ON D.AppID=A.AppID " & vbCrLf & _
		"	LEFT JOIN (SELECT AppID, SUM(CashMatch) AS CashMatch FROM " & ApplicationSchema & ".BudgetDetails GROUP BY AppID) AS M ON M.AppID=A.AppID " & vbCrLf & _
		"	LEFT JOIN [Grants].Main AS PY ON PY.GranteeID=A.GranteeID AND PY.FiscalYear=A.FiscalYear-1 AND PY.GrantClassID=I.GrantClassID " & vbCrLf & _
...
If CreateGrantRecord = True Or CreateStubRecord = True Then
	...
	"JOIN [Application].Admin AS B ON B.AppID=A.AppID " & vbCrLf & _
	...
	' *******************************************************
	If True = False Then ' Cancelled for now. They will get all questions.
		sql = "INSERT INTO PR.GrantQuestions (" & vbCrLf & _
			"SELECT C.GrantID, B.QuestionID, A.IntegerResponse AS IntegerTarget, A.DecimalResponse AS DecimalTarget " & vbCrLf & _
			"FROM " & ApplicationSchema & ".GSATargets AS A " & vbCrLf & _
			"LEFT JOIN PR.Activities AS B ON B.GoalID=A.GoalID AND B.StrategyID=A.StrategyID AND B.ActivityID=A.ActivityID AND B.MeasureID=A.MeasureID " & vbCrLf & _
			"LEFT JOIN [Grants].Main AS C ON C.AppID=A.AppID " & vbCrLf & _
			"LEFT JOIN PR.GrantQuestions AS D ON D.GrantID=C.GrantID AND D.QuestionID=B.QuestionID " & vbCrLf & _
			"WHERE C.AppID=" & prepIntegerSQL(AppID) & " AND (A.IntegerResponse IS NOT NULL OR A.DecimalResponse IS NOT NULL) AND D.GrantID IS NULL "
		If Debug = True Then
			Response.Write("<pre>Create Grant Record." & vbCrLf & sql & "</pre>")
			Response.Flush
		End If
...
If ClearSubmit = True Then
	sql = "UPDATE Application.Main " & vbCrLf & _
...
If RevisedClearSubmit = True Then
	sql = "UPDATE " & ApplicationSchema & ".Main " & vbCrLf & _
		"SET ConfirmationNumber=null, SubmitID=null, SubmitTimeStamp=null, UpdateID=" & prepIntegerSQL(UserSystemID) & ", UpdateTimestamp=" & prepStringSQL(Timestamp) & " " & vbCrLf & _
		"WHERE AppID=" & prepIntegerSQL(AppID)
	If Debug = True Then
		Response.Write("<pre>" & sql & "</pre>" & vbCrLf)
