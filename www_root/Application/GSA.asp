...
Dim debug, i, j, k, PermitEdit, Version, AppID, GrantClassID, LastGoal, LastStrategy, LastMandatory,  _
	FiscalYear, GranteeID, GranteeName, GrantClass, _
	SubmitID, SubmitByName, SubmitTimestamp, AppURL, ApplicationSchema 
...
<html lang="en-us">
<head>
<title>MVCPA <%=GrantClass %> <%=ApplicationSchema %> for <%=GranteeName %></title>
...
<h1><%=GranteeName %> MVCPA <%=GrantClass %>&nbsp;<%=ApplicationSchema %> for Fiscal Year <%=FiscalYear %></h1>
...
<%=HiddenField("AppID", AppID) %><%=HiddenField("GrantClassID", GrantClassID)%><%=HiddenField("Version", Version) %><%=HiddenField("FiscalYear", FiscalYear) %>
