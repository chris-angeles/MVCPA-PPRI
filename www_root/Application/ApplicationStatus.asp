...
If FiscalYear > 0 Then
	If getApplicationSchema(FiscalYear) = "Negotiation" Then 
...
If Debug = True Then
	Response.Write("<pre>Set up SQL Statement.</pre>")
	Response.Flush
End If
...
If AppToShow = 0 Or AppToShow = 2 Then
	sql = sql & ", [Submitted By], [Submitted], [Application Certified Complete Date], " & vbCrLf & _
...
If ShowAgreements = True Then
	...
	"	[Operational Plan Approved], [Operational Plan], [Multi-Agency Grant] " & vbCrLf
Else
	sql = sql & vbCrLf
End If
...
Else
End If
sql = sql & "FROM Application.vwSummary " & vbCrLf & _
	"WHERE GrantClassID=1 AND [Fiscal Year]=" & prepIntegerSQL(FiscalYear) & " " & vbCrLF
If AppToShow = 1 Or AppToShow = 2 Then
...
<div class="sectiontitle" style="white-space: nowrap;"><%=FiscalYear%> Taskforce Grant Applications</div>
		...
		For i = 0 To (rs.Fields.Count-1)
			...
			ElseIf rs.Fields(i).Name = "App ID" And ShowExcel = False Then
				If MVCPARights = True Then
					If AppToShow = 0 Then
						If FiscalYear > 2021 Then
							Response.Write("<td style=""text-align: right"" title=""View/Print Application""><a href=""https://" & Request.ServerVariables("SERVER_NAME")& "\Application\TFGPrintApplication.asp?AppID=" & rs.Fields(i) & "&FiscalYear=" & FiscalYear & """ target=""_blank"" class=""plainlink"">" & rs.Fields(i) & "</a></td>" & vbCrLf)
						Else
							Response.Write("<td style=""text-align: right"" title=""View/Print Application""><a href=""https://" & Request.ServerVariables("SERVER_NAME")& "\Application\PrintApplication.asp?AppID=" & rs.Fields(i) & "&FiscalYear=" & FiscalYear & """ target=""_blank"" class=""plainlink"">" & rs.Fields(i) & "</a></td>" & vbCrLf)
						End If
					Else
						If FiscalYear > 2021 Then
							Response.Write("<td style=""text-align: right"" title=""View/Print Negotiation Application""><a href=""https://" & Request.ServerVariables("SERVER_NAME")& "\Negotiation\TFGPrintApplication.asp?AppID=" & rs.Fields(i) & "&FiscalYear=" & FiscalYear & """ target=""_blank"" class=""plainlink"">" & rs.Fields(i) & "</a></td>" & vbCrLf)
						Else
							Response.Write("<td style=""text-align: right"" title=""View/Print Negotiation Application""><a href=""https://" & Request.ServerVariables("SERVER_NAME")& "\Negotiation\PrintApplication.asp?AppID=" & rs.Fields(i) & "&FiscalYear=" & FiscalYear & """ target=""_blank"" class=""plainlink"">" & rs.Fields(i) & "</a></td>" & vbCrLf)
						End If
					End If
					...
					Response.Write("<td style=""text-align: right"" title=""Manage Application""><a href=""https://" & Request.ServerVariables("SERVER_NAME")& "\Application\AppAdmin.asp?AppID=" & rs.Fields(i) & "&FiscalYear=" & FiscalYear & """ target=""_blank"" class=""plainlink"">M</a></td>" & vbCrLf)
		...
		If ResolutionStatus = True Then
			...
			If fso.FolderExists(DocumentFolder) Then
				If fso.FileExists(DocumentFolder & "Resolution.pdf") Then
					Response.Write("<td><a href=""https://" & Request.ServerVariables("SERVER_NAME")& "/Documents/Application/" & rs.Fields("App ID") & "/Resolution.pdf"" target=""_blank"">Res</a></td>")
				Else
