...
If Debug = True Then
	...
	Response.Write(vbCrLf & "<pre>button=" & Button & "</pre>" & vbCrLf)
	Response.Write(vbCrLf & "<pre>AppID=" & AppID & "</pre>" & vbCrLf)
End If

If AppID=0 Or Button="submit" Or foundchange = true Then
	If AppID = 0 Then
		sql = "SELECT AppID, GrantClassID, FiscalYear, GranteeID " & vbCrLf & _
			"FROM Application.IDs " & vbCrLf & _
			"WHERE GrantClassID=1 AND FiscalYEar=" & prepIntegerSQL(FiscalYear) & " AND GranteeID=" & _
	...
	sql = "SELECT AppID FROM Application.Main WHERE AppID=" & prepIntegerSQL(AppID)
	If Debug = True Then
		Response.Write("<pre>" & sql & "</pre>" & vbCrLf)
		Response.Flush
	End If
	Set rs = Con.Execute(sql)
	If rs.EOF = True Then
		sql = "INSERT INTO " & ApplicationSchema & ".Main (AppID, ProgramName, CoverageAreaDescription, GrantTypeID, " & vbCrLf & _
		...
		Con.Execute(sql)
	Else
		sql = "UPDATE " & ApplicationSchema & ".Main " & vbCrLf & "SET " & _
...
If Button = "save" Then
	If Debug = True Then
		Response.Write("<a href=""TFGApplication.asp?GranteeID=" & GranteeID & "&AppID=" & AppID & _
			"&FiscalYear=" & FiscalYear & """>return</a>")
	Else
		Response.Redirect("TFGApplication.asp?GranteeID=" & GranteeID & "&AppID=" & AppID & _
...
ElseIf Button = "GSA" Then
	If Debug = True Then
		Response.Write("<a href=""gsa.asp?AppID=" & AppID & """>Goals Strategies and Activities</a>")
	Else
		Response.Redirect("gsa.asp?AppID=" & AppID)
	End If
...
ElseIf Button="Statistics" Then
	If Debug = True Then
		Response.Write("<a href=""Statistics.asp?AppID=" & AppID & """>Statistics</a>")
	Else
