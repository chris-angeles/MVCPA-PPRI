...
Dim MAXQUARTER
Dim debug, i, j, PermitEdit, ViewDocuments, UploadDirectory, PermitMVCPADeduction, _
	GrantID, FiscalYear, GrantClassID, GrantClass, Quarter, GranteeID, GranteeName, ProgramName, GrantNumber, _
	DisplayQuarterOffset, Confirmed, SubmitID, SubmitTimeStamp, SubmitName, SubmitterEMail, _
	...
	InLieuOfNICBBudget, ProgramIncomeBudget, _
	InLieuOfNICB, UnbudgetedPI, _
	...
	SupplementaryComments, PriorInLieuOfNICB, PriorAmountPaid, _
	CurrentSaleOfAssets, PriorSaleOfAssets, CurrentMVCPADeduction, PriorMVCPADeduction, PriorProgramIncome, _
...
sql = "SELECT A.GranteeID, B.GrantID, B.FiscalYear, B.GrantClassID, GC.GrantClass, ISNULL(C.Quarter, " & prepIntegerSQL(Quarter) & ") AS Quarter , " & vbCrLf & _
	"	A.GranteeName, B.ProgramName, B.GrantNumber, B.DisplayQuarterOffset, " & vbCrLf & _
	...
	"	CASE WHEN C.DatePaid IS NULL AND C.Reimbursement<0 THEN 0.0 " & vbCrLf & _
	"		WHEN DatePaid IS NOT NULL THEN C.AmountPaid " & vbCrLf & _
	"		WHEN C.SubmitID IS NOT NULL AND C.AmountPaid IS NULL THEN C.Reimbursement " & vbCrLf & _
	"		ELSE C.AmountPaid END AS AmountPaid, " & vbCrLf & _
	...
	"	ISNULL(C.CurrentSaleOfAssets,0.0) AS CurrentSaleOfAssets, " & vbCrLF & _
	"	ISNULL(C.CurrentMVCPADeduction,0.0) AS CurrentMVCPADeduction, " & vbCrLf & _
	"	ISNULL(C.Reimbursement,0.0) AS Reimbursement, " & vbCrLf & _
	...
	"	CASE WHEN C.SubmitID>0 THEN ISNULL(C.PriorSaleOfAssets, 0.0) ELSE ISNULL(J.PriorSaleOfAssets, 0.0) END AS PriorSaleOfAssets, " & vbCrLf & _
	"	CASE WHEN C.SubmitID>0 THEN ISNULL(C.PriorMVCPADeduction, 0.0) ELSE ISNULL(J.PriorMVCPADeduction, 0.0) END AS PriorMVCPADeduction, " & vbCrLf & _
	...
	"	ISNULL(J.PriorNonApproval,0) AS PriorNonApproval, " & vbCrLF & _
	"	ISNULL(N.LaterSubmission,0) AS LaterSubmission, " & vbCrLf & _
	...
	"LEFT JOIN (SELECT GrantID, " & vbCrLf & _
	"	SUM(InLieuOfNICB) AS PriorInLieuOfNICB, SUM(AmountPaid) AS PriorAmountPaid, " & vbCrLf & _
	"	SUM(CurrentSaleOfAssets) AS PriorSaleOfAssets, " & vbCrLf & _
	"	SUM(CurrentMVCPADeduction) AS PriorMVCPADeduction, " & vbCrLF & _
	"	CAST(MAX(CASE WHEN DirectorApprovalID IS NULL THEN 1 ELSE 0 END) AS BIT) AS PriorNonApproval " & vbCrLf & _
	"	FROM ER.Main WHERE Quarter<" & prepIntegerSQL(Quarter) & vbCrLf & _
	"	GROUP BY GrantID) AS J ON J.GrantID=B.GrantID " & vbCrLf & _
	...
	"LEFT JOIN Lookup.GrantClass AS GC ON GC.GrantClassID=B.GrantClassID " & vbCrLf & _
...
If MVCPAAdministrator = True And SubmitID > 0 And DirectorApprovalID = 0 Then
	PermitMVCPADeduction = True
Else
	PermitMVCPADeduction = False
End If
...
If MVCPAViewer = True Or MVCPAAuditor=True Then
	ViewDocuments = True
End If
...
%>];
	...
	rs.MoveFirst
%>];
	var reimbursementrate =<%=ReimbursementRate%>, awardamount =<%=awardamount %>, prioramountpaid =<%=PriorAmountPaid %>, prioryearallocation =<%=PriorYearAllocation %>, PriorSaleOfAssets =<%=PriorSaleOfAssets %>, PriorMVCPADeduction = <%=PriorMVCPADeduction%>;
...
<%	If GrantClassID = 1 Then %>
		...
		reimbursabletotal = reimbursabletotal - getNumericValue(document.ER["PriorInLieuOfNICB"].value) -
			getNumericValue(document.ER["InLieuOfNICB"].value) - getNumericValue(document.ER.UnbudgetedPI.value);
<%	Else %>
		reimbursabletotal = reimbursabletotal - getNumericValue(document.ER["PriorInLieuOfNICB"].value) -
			getNumericValue(document.ER["InLieuOfNICB"].value);
<%	End If %>
		...
		if (reimbursementytd>awardamount) 
			reimbursementytd = awardamount;
		document.ER.ReimbursementYTD.value = currency(reimbursementytd);

		reimbursement = reimbursementytd - prioramountpaid - PriorSaleOfAssets - PriorMVCPADeduction - getNumericValue(document.ER["CurrentSaleOfAssets"].value) - getNumericValue(document.ER["CurrentMVCPADeduction"].value);
...
<%	End If %>
		...
		if (getNumericValue(document.ER.Reimbursement.value) > 0.0)
			document.ER.AmountPaid.value = document.ER.Reimbursement.value;
		else if(Len(document.ER.DatePaid.value) == 0)
			document.ER.AmountPaid.value = "$0.00";
...
<%	End If %>
	...
	function validateForm(action)
	{
		...
		if (checkCurrency(document.ER["CurrentSaleOfAssets"]) == false)
			return false;
...
%>
<br />
<form name="ER" id="ER" method="post" action="ReportSubmit2.asp">
<%=HiddenField("GrantID",GrantID) %><%=HiddenField("GrantClassID",GrantClassID) %><%=HiddenField("LoadedQuarter",Quarter) %><%=HiddenField("Action","save") %><%=HiddenField("PriorYearAllocation", PriorYearAllocation) %><%=HiddenField("AwardAmount",AwardAmount) %>
<table>
	<tr><td>Grantee Name</td><td><%=GranteeName %></td></tr>
	<tr><td>Grant Class</td><td><%=GrantClass %></td></tr>
...
<%
	for i = 1 to MAXQUARTER
		If GrantClassID = 4 And FiscalYear=2024 Then
			Response.Write(vbTab & SelectOption(i, "Q" & i & "", Quarter))
		Else
			Response.Write(vbTab & SelectOption(i, ReportingPeriodDates(FiscalYear, i+DisplayQuarterOffset) & " (Q" & i & ")", Quarter))
		End If
	next
...
<table>
	...
	<thead>
	<tr>
		<th rowspan="2" style="white-space: nowrap;">Budget Category</th>
...
<table style="margin: auto; ">
	...
	<tr>
		...
		<td style="text-align: center; "><%=CurrencyField("PriorSaleOfAssets", PriorSaleOfAssets, 12, 15, False, "") %></td>
	</tr>
	<tr>
		<td>less Current Sale of Assets</td>
		<td style="text-align: center; "><%=CurrencyField("CurrentSaleOfAssets", CurrentSaleOfAssets, 12, 15, PermitEdit, "checkCurrency(this); updateTotals()") %></td>
	</tr>
	<tr>
		<td>less Prior MVCPA Deductions</td>
		<td style="text-align: center; "><%=CurrencyField("PriorMVCPADeduction", PriorMVCPADeduction, 12, 15, False, "") %></td>
	</tr>
	<tr>
		<td>less Current MVCPA Deductions</td>
		<td style="text-align: center; "><%=CurrencyField("CurrentMVCPADeduction", CurrentMVCPADeduction, 12, 15, PermitMVCPADeduction, "checkCurrency(this); updateTotals()") %></td>
	</tr>
...
<%	If GrantClassID=4 Then 
			HiddenField "BeginningBalance", 0.00
			HiddenField "EarnedThisQuarter", 0.00
			HiddenField "ExpendedThisQuarter", 0.00
			HiddenField "EndingBalance", 0.00
	Else %>
<br />
<table style="margin: auto; ">
...
If MVCPARights = True Or MVCPAViewer = True Then
	...
	If rs.EOF = False Then
		If Quarter = 1 And Len(rs.Fields("ER Q1")) = 0 Then
			Response.Write("<div style=""text-align: center; color: red; "">First Quarter Expenditure Report has not been submitted.</div>" & vbCrLf)
			CanApprove = False
			CanInvoice = False
		End If
		If Quarter = 2 And Len(rs.Fields("ER Q2")) = 0 Then
			Response.Write("<div style=""text-align: center; color: red; "">Second Quarter Expenditure Report has not been submitted.</div>" & vbCrLf)
			CanApprove = False
			CanInvoice = False
		End If
		If Quarter = 3 And Len(rs.Fields("ER Q3")) = 0 Then
			Response.Write("<div style=""text-align: center; color: red; "">Third Quarter Expenditure Report has not been submitted.</div>" & vbCrLf)
			CanApprove = False
			CanInvoice = False
		End If
		If Quarter = 4 And Len(rs.Fields("ER Q4")) = 0 Then
	...
	If SubmitID > 0 Then
		...
		Else
			Response.Write("<tr><td>Amount To Pay</td><td>" & CurrencyField("AmountPaid", AmountPaid, 12, 15, False, "checkCurrency(this); updateTotals();") & "</td>" & vbCrLf)
		End If
		...
		Else
			...
			If CanInvoice = True Then
				Response.Write(" <a href=""Voucher2.asp?GrantID=" & GrantID & "&Quarter=" & Quarter & """ target=""_blank"">invoice</a>" & vbCrLf)
			End If
			Response.Write("</td></tr>" & vbCrLf)
		End If
...
<%	If PermitEdit = True or SubmitID = 0 Or MVCPARights = True Or PermitMVCPADeduction = True Then %>
...
<%	ElseIf SubmitID = 0 And CanSubmit=True And Date()<StartDate  Then 
%>
