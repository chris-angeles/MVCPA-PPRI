...
Dim debug, i, j, TimeStamp, GrantID, GrantClassID, Quarter, Action, Submit, Unsubmit, _
CashExpenditureTotal, InKindExpenditureTotal, InLieuOfNICB, UnbudgetedPI, _
ReimbursableExpenditures, ReimbursementRate, ReimbursementYTD, PriorAmountPaid, Reimbursement, _
PriorYearAllocation, AwardAmount, CurrentSaleOfAssets, PriorSaleOfAssets, _
CurrentMVCPADeduction, PriorMVCPADeduction, _
InLieuOfNICBBudget, ProgramIncomeBudget, PriorInLieuOfNICB, PriorProgramIncome, _
...
End If
If Len(GrantClassID) = 0 Then
	Response.Write("Error: No GrantClassID provided.")
	sendWarning("Error: No GrantClassID provided.")
	Response.End
Else
	GrantClassID = CInt(GrantClassID)
End If
...
PriorSaleOfAssets = Request.Form("PriorSaleOfAssets")
CurrentSaleOfAssets = Request.Form("CurrentSaleOfAssets")
CurrentMVCPADeduction = Request.Form("CurrentMVCPADeduction")
PriorMVCPADeduction = Request.Form("PriorMVCPADeduction")
...
sql = "SELECT GrantID, Quarter, CashExpenditureTotal, InKindExpenditureTotal, " & vbCrLf & _
	"	InLieuOfNICB, UnbudgetedPI, " & vbCrLf & _
	...
	"	InLieuOfNICBBudget, ProgramIncomeBudget, PriorInLieuOfNICB, PriorProgramIncome, " & vbCrLf & _
	"	PriorSaleOfAssets, CurrentSaleOfAssets, CurrentMVCPADeduction, PriorMVCPADeduction, " & vbCrLf & _
...
If rs.EOF = True Then
	...
	sql = "INSERT INTO ER.Main (GrantID, Quarter, CashExpenditureTotal, InKindExpenditureTotal, " & vbCrLf & _
		"	InLieuOfNICB, UnbudgetedPI, ReimbursableExpenditures, " & vbCrLf & _
		...
		"	InLieuOfNICBBudget, ProgramIncomeBudget, " & vbCrLf & _
		"	PriorInLieuOfNICB, PriorProgramIncome, PriorSaleOfAssets, CurrentSaleOfAssets, CurrentMVCPADeduction, PriorMVCPADeduction, " & vbCrLf & _
		...
		prepNumberSQL(PriorSaleOfAssets) & ", " & _
		prepNumberSQL(CurrentSaleOfAssets) & ", " & _
		prepNumberSQL(CurrentMVCPADeduction) & ", " & _
		prepNumberSQL(PriorMVCPADeduction) & ", " & _
...
Else
	sql = "UPDATE ER.Main SET " & vbCrLf & _
		...
		"PriorSaleOfAssets=" & prepnumberSQL(PriorSaleOfAssets) & ", " & vbCrLf & _
		"CurrentSaleOfAssets=" & prepNumberSQL(CurrentSaleOfAssets) & ", " & vbCrLf & _
...
IF MVCPARights = True Then
	sql = "UPDATE ER.Main SET " & vbCrLf & _
		...
		"	CurrentMVCPADeduction=" & prepNumberSQL(CurrentMVCPADeduction) & ", " & vbCrLf & _
		"	PriorMVCPADeduction=" & prepNumberSQL(PriorMVCPADeduction) & ", " & vbCrLf & _
...
Else
	If Debug = True Then
		Response.Write("<pre>The Program Income section is being skipped because this is not a Taskforce Grant.</pre>" & vbCrLF)
	End If
End If
