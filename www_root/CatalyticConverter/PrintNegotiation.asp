...
Dim debug, i, j, LastCategory, PermitEdit, AllowUpload, SectionTextRequired, SectionTextFix, _
	...
	TargetAwardAmount, TargetMatchAmount, TargetReimbursementRate, GSATargets, ViewApplication
...
Else
	...
	sql = sql & "LEFT JOIN System.Users AS U ON U.SystemID=A.SubmitID " & vbCrLf & _
		...
		"	FROM " & ApplicationSchema & ".Matches " & vbCrLf & _
		"	GROUP BY AppID) AS M ON M.AppID=I.AppID " & vbCrLf & _
		"WHERE G.GranteeID=" & PrepIntegerSQL(GranteeID) & " AND ISNULL(I.FiscalYear," & PrepIntegerSQL(FiscalYear) & ")=" & PrepIntegerSQL(FiscalYear)
End If
If Debug = True Then
...
If FiscalYear=2024 And Now()>CDate("03/15/2024 5:00:00 PM") Then
	PermitEdit = False
	AllowUpload = True
...
<br />
<%
If AppID>0 And (AllowUpload = True) And ViewApplication = False Then
