...
Dim debug, i, j, LastCategory, PermitEdit, AllowUpload, SectionTextRequired, SectionTextFix, _
	ApplicationSchema, NegotiationLocked, AppID, FiscalYear, GranteeID, GranteeName, _
	...
	ProgramCategory1, ProgramCategory2, ProgramCategory3, ProgramCategory5, HistoricalDataYear, _
...
If AppID>0 Then 
	sql = "SELECT G.GranteeID, G.GranteeName, G.ORI, ORI.Agency AS ORIAgency, G.OrganizationTypeID, OT.OrganizationType, G.StatePayeeIDNo, " & vbCrLf & _
		"	ISNULL(I.FiscalYear, " & prepIntegerSQL(FiscalYear) & ") AS FiscalYear, /*ISNULL(NegotiationLocked,0) AS NegotiationLocked,*/ " & vbCrLf & _
		...
		"	ProgramCategory1, ProgramCategory2, ProgramCategory3, ProgramCategory5, ISNULL(HistoricalDataYear," & (FiscalYear-1) & ") AS HistoricalDataYear, " & vbCrLf & _
		...
		"LEFT JOIN Application.IDs AS I ON I.GranteeID=G.GranteeID AND ISNULL(GrantClassID,4)=4" & vbCrLf & _
...
Else
	sql = "SELECT G.GranteeID, G.GranteeName, G.ORI, ORI.Agency AS ORIAgency, G.OrganizationTypeID, OT.OrganizationType, G.StatePayeeIDNo, " & vbCrLf & _
		"	ISNULL(I.FiscalYear, " & prepIntegerSQL(FiscalYear) & ") AS FiscalYear, /*ISNULL(NegotiationLocked,0) AS NegotiationLocked,*/ " & vbCrLf & _
		"	A.GrantTypeID, AuthorizedOfficialID, AO.Name AS AuthorizedOfficial, AO.Title AS AuthorizedOfficialTitle, ISNULL(A.AppID,0) AS AppID, ProgramName, " & vbCrLf & _
		"	A.CoverageAreaDescription, A.StatewideCoverage, A.OtherCoverage, A.OtherCoverageText, A.LawEnforcementGrant, " & vbCrLf & _
		"	NationalInsuranceCrimeBureau, TexasDepartmentOfPublicSafety, OtherAgency, OtherAgencySpecify, " & vbCrLf & _
		"	ProgramCategory1, ProgramCategory2, ProgramCategory3, ProgramCategory5, ISNULL(HistoricalDataYear," & (FiscalYear-1) & ") AS HistoricalDataYear, " & vbCrLf & _
		"	A.Certification, A.SubmitID, U.Name AS SubmitByName, A.SubmitTimestamp, A.ConfirmationNumber, " & vbCrLf & _
		...
		"FROM Grantees AS G " & vbCrLf & _
		"LEFT JOIN Application.IDs AS I ON I.GranteeID=G.GranteeID AND ISNULL(GrantClassID,4)=4 AND ISNULL(I.FiscalYear," & PrepIntegerSQL(FiscalYear) & ")=" & PrepIntegerSQL(FiscalYear) & " " & vbCrLf & _
		"LEFT JOIN CC." & ApplicationSchema & " AS A ON A.AppID=I.AppID " & vbCrLf & _
...
Else
	...
	ProgramCategory(1) = rs.Fields("ProgramCategory1")
	ProgramCategory(2) = rs.Fields("ProgramCategory2")
	ProgramCategory(3) = rs.Fields("ProgramCategory3")
	ProgramCategory(5) = rs.Fields("ProgramCategory5")
...
If Debug = True Then
	Response.Write("<pre>ReadyToSubmit=" & ReadyToSubmit & "</pre>" & vbCrLf)
End If
DocumentFolder = Application("DocumentRoot") & "\Application\" & AppID & "\"

' the following lines can be used to lock the application.
If Now() > getDeadline(FiscalYear, ApplicationSchema) And AppID <> 279 And AppID <> 342 And GranteeID <> 190 And GranteeID <> 136  And GranteeID <> 229 And GranteeID <> 6  And GranteeID <> 109 And GranteeID <> 80 And GranteeID <> 114 Then
	PermitEdit = False
	AllowUpload = True
...
If Debug = True Then
	Response.Write("<pre>FiscalYear=" & FiscalYear & "</pre>" & vbCrLf)
	Response.Write("<pre>ApplicationSchema=" & ApplicationSchema & "</pre>" & vbCrLf)
	Response.Write("<pre>PermitEdit=" & PermitEdit & "</pre>" & vbCrLf)
	Response.Write("<pre>Deadline=" & getDeadline(FiscalYear, ApplicationSchema) & "</pre>" & vbCrLf)
End If
...
<html lang="en-us">
<head>
<title>Senate Bill 224 Grant <%=ApplicationSchema %></title>
...
<div class="pagetag">Senate Bill 224 Grant <%=ApplicationSchema %> for <%=FiscalYear %></div>

<div class="widecontent">

<form name="Application" id="Application" method="post" action="<%
If ApplicationSchema = "Application" Then
	Response.Write("ApplicationSubmit.asp")
ElseIf ApplicationSchema = "Negotiation" Then
	Response.Write("NegotiationSubmit.asp")
End If
...
<%	
Else
	Response.Write("<tr><td colspan=""2"" style=""text-align: center; "">" & getRFALink(FiscalYear) & "</td></tr>" & vbCrLf)
End If 
...
End If
'Response.Write(vbCrLf & "here" & vbCrLf)
'Response.Flush
%>
...
<%
	If FiscalYear=2024 Then
		sql = "SELECT GrantTypeID, GrantType, GrantTypeDescription FROM Lookup.GrantType WHERE GrantTypeID=3 AND Version=3 "
	Else
		' Only new in 2024. In future years, allow for new and continued.
		sql = "SELECT GrantTypeID, GrantType, GrantTypeDescription FROM Lookup.GrantType WHERE GrantTypeID=3 AND Version=1 "
	End If
	...
	Set rs = Con.Execute(sql)
	While rs.EOF = False
		Response.Write(vbTab & "<tr style=""vertical-align: top""><td>" & RadioInputField2("GrantTypeID", GrantTypeID, rs.Fields("GrantTypeID"), PermitEdit) & "</td><td><b>" & _
...
<%
	sql = "SELECT ProgramCategoryID, ProgramCategory FROM Lookup.ProgramCategory WHERE Version=2 ORDER BY ProgramCategoryID "
	Set rs = Con.Execute(sql)
	While rs.EOF = False
		Response.Write(vbTab & "<tr><td>" & CheckboxField2("ProgramCategory" & rs.Fields("ProgramCategoryID"), ProgramCategory(rs.Fields("ProgramCategoryID")), PermitEdit) & _
...
<tr>
	...
	of the Catalytic Converter program:</b></br> 
...
<tr style="vertical-align: top; ">
	...
	that their agency will coordinate or call upon the program. Letters of support with the 
...
<tr style="vertical-align: top; ">
	<td><%=CheckBoxField2("OtherCoverage", OtherCoverage, PermitEdit) %></td>
...
<tr style="vertical-align: top; ">
	<td><%=CheckBoxField2("NationalInsuranceCrimeBureau", NationalInsuranceCrimeBureau, PermitEdit) %></td>
	<td><b>National Insurance Crime Bureau (NICB)</b> Used as Match (Documentation and time certification required.)</td>
</tr>

<tr style="vertical-align: top; ">
	<td><%=CheckBoxField2("TexasDepartmentOfPublicSafety", TexasDepartmentOfPublicSafety, PermitEdit) %></td>
	<td><b>Texas Department of Public Safety (DPS)</b></td>
</tr>

<tr style="vertical-align: top; ">
	<td><%=CheckBoxField2("OtherAgency", OtherAgency, PermitEdit) %></td>
	...
	While rs.EOF = False
		...
		ElseIf PermitEdit = False Then
			If BudgetEntryOption = 2 Then
				Response.Write(vbTab & "<td><a href=""/Application/BudgetDetail2.asp?AppID=" & AppID & _
					"&BudgetCategoryID=" & rs.Fields("BudgetCategoryID") & """ class=""plainlink"">" & rs.Fields("BudgetCategory") & "</a></td>" & vbCrLf)  
			Else
				Response.Write(vbTab & "<td><a href=""/Application/BudgetDetail.asp?AppID=" & AppID & _
					"&BudgetCategoryID=" & rs.Fields("BudgetCategoryID") & """ class=""plainlink"">" & rs.Fields("BudgetCategory") & "</a></td>" & vbCrLf)  
			End If
		Else
...
<%
	...
	Else
		Response.Write("<div style=""text-align: center""><a href=""/Application/Matches.asp?AppID=" & AppID & "&MatchTypeID=1"" class=""plainlink"">Cash Match</a></div>" & vbCrLf)
	End If
	...
	Else
		Response.Write("<div style=""text-align: center""><a href=""/Application/Matches.asp?AppID=" & AppID & "&MatchTypeID=2"" class=""plainlink"">In-Kind Match</a></div>" & vbCrLf)
	End If
...
<thead>
	...
	<tr style="vertical-align: bottom; ">
		...
		<th style="width: 115px; ">Catalytic Converter Theft</th>
		<th style="width: 115px; ">Catalytic Converter Theft</th>
	</tr>
</thead>
<tbody>
...
While rs.EOF = False
	...
	Response.Write(vbTab & vbTab & "<td style=""text-align: right; "">" & formatInteger(rs.Fields("CCTheft1")) & "</td>" & vbCrLf)
	Response.Write(vbTab & vbTab & "<td style=""text-align: right; "">" & formatInteger(rs.Fields("CCTheft2")) & "</td>" & vbCrLf)
...
While rs.EOF = False
	If rs.Fields("SpecialTreatments") = "GSA" Then 
		...
		Else
			Response.Write("<p style=""text-align: left""><a href=""/Application/GSA.asp?AppID=" & AppID & """ class=""plainlink"">Select Goals, Strategies, and Activity Targets</a> for the proposed program described in the application.</p>")
			Response.Write("<p>Click on the link above to open a new table called Goals, Strategies, and Activity. Applicants will review the statutory and optional activities. The first section is the statutory measures. Grantees may not provide targets for this section but need to review the descriptions to ensure that they understand the statutory minimum requirements associated with this grant. Applicants will then review the other items and estimate targets for only the activities that the applicant has described and plans to complete under this application.</p>")
		End If
...
<!--#include file="../includes/CCCommonFunctions.asp"-->
